...
const SideBar = () => {
  ...
  return (
    <div className='fixed w-[250px] bg-[#ffffff] flex flex-col items-start h-full pt-6 gap-6'>
        <Link href="/searchbyname" onClick={()=>setShowAllEntities(false)} className={`h-[40px] ml-4 no-underline flex items-center justify-center ${path==="/searchbyname"&&!showAllEntities&&'side-bar-btn-active'}`}>Rechrche par nom</Link>
        <Link href="/searchbyid" onClick={()=>setShowAllEntities(false)} className={`h-[40px] ml-4 no-underline flex items-center justify-center ${path==="/searchbyid"&&!showAllEntities&&'side-bar-btn-active'}`}>Rechrche par ID</Link>
